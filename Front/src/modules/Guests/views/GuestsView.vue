<template>
  <div class="guests-view">
    ...
    <div class="guests-view__content">
      ...
      <template #item.bookings="{item,value}">
  ...
  <v-dialog v-model="dialogVisible" max-width="500">
    <v-card
      :subtitle="isEditing ? 'Edycja osoby' : 'Dodaj osobe'"
...
async function openDeleteDialog(id: number): Promise<void> {
  ...
  const title = 'Usuwanie osoby';
  const message = `Czy na pewno chcesz usunąć osobę "${guest.name, guest.surname}"?`;
